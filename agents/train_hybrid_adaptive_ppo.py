...
==========================
Adaptive hybrid penalty: hard unsafe-action masking + scheduled risky penalty.
...
Hybrid behavior:
  ...
  * Risky actions remain available and receive a penalty that starts low,
    then ramps up over training to preserve early exploration.
"""
...
from env.lava_hybrid_adaptive_wrapper import make_hybrid_env, make_hybrid_video_env
from metrics.hybrid_adaptive_training_logger import HybridEpisodeCSVLogger
...
def make_video_env(env_id: str, video_folder: str, seed: int) -> gym.Env:
    return make_hybrid_video_env(
        ...
        video_folder,
        seed=seed,
        risky_penalty=RISKY_PENALTY_END,
    )
...
def evaluate_hybrid_model(
    ...
    env = make_hybrid_env(env_id, seed=seed, risky_penalty=RISKY_PENALTY_END)
...
def summarize_training(
    ...
    if len(df) == 0:
        return {
            ...
            "final_risky_penalty": float("nan"),
            "convergence_episode": None,
            "convergence_timestep": None,
        }

    final_risky_penalty = (
        float(df["episode_final_risky_penalty"].dropna().iloc[-1])
        if "episode_final_risky_penalty" in df.columns
        and not df["episode_final_risky_penalty"].dropna().empty
        else RISKY_PENALTY_END
    )
    ...
    return {
        ...
        "final_risky_penalty": final_risky_penalty,
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        for env_id in ENV_IDS:
            ...
            print(
                f"\n=== Training (Adaptive Hybrid PPO) | {env_id} | seed={seed} | "
                f"penalty={RISKY_PENALTY_START}->{RISKY_PENALTY_END} "
                f"over {RISKY_PENALTY_SCHEDULE_STEPS} steps ==="
            )
